...
def get_best_model(training_data, test_data):
    ...
    metricName = 'rmse'
    tvs_list = get_trainers(.8, estimator_gridbuilders, feature_cols, label_cols, metricName)
...
def main():
    ...
    round_float = functions.udf(lambda float_value: round(float_value))
    ...
    for attribute in training_attributes:
        ...
        data_pred = model.transform(test_data_with_feature)
        data_pred = data_pred.drop('feature_cols')
        data_pred = data_pred.withColumn('{}_pred'.format(attribute), round_float(data_pred['{}_pred'.format(attribute)]))
        df_attributes.append(data_pred.select('id', '{}_pred'.format(attribute), '{}'.format(attribute)))
